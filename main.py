...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            dog_race_pred = dog_race("static/" + filename)
            redirect(url_for('upload_file', filename=filename))
        return render_template('index.html', dog_race_pred=dog_race_pred,filename=filename)
...
yolo_model = load_model("./models/YOLO/", compile=False)

def dog_race(image_path):
    ...
    for i, j in enumerate(out_classes):
        if j == 16:  # 16 est la classe chien de YOLO
            ...
            prediction = classifier.predict(image_dog)[0]
            top3 = prediction.argsort()[::-1][:3]
            top3_list.append("Dog "+str(count)+" = "+ str(np.array(out_scores)[i]))
            count+=1
            for i in top3:
...
class_names = read_classes("models/coco_classes.txt")
anchors = read_anchors("models/yolo_anchors.txt")
...
def draw_boxes(image, boxes, box_classes, class_names, scores=None):
    ...
    font = ImageFont.truetype(
        font= 'models/font/FiraMono-Medium.otf',
